...
impl<T: Translation<A>, A: PagingAttributes> PageTableWithLevel<T, A> {
    ...
    fn map_range(
        ...
        for chunk in range.split(level) {
            ...
            } else if chunk.is_block(level) && !flags.contains(A::VALID) {
                if let Some(mut subtable) = entry.subtable(translation, level) {
                    // There is a subtable but we can remove it. To avoid break-before-make
                    // violations this is only allowed if the new mapping is not valid, i.e. we
                    // are unmapping the memory.
                    entry.set(PhysicalAddress(0), flags);

                    // SAFETY: The subtable was created with the same translation by
                    // `PageTableWithLevel::new`, and is no longer referenced by this table. We
                    // don't reuse subtables so there must not be any other references to it.
                    unsafe {
                        subtable.free(translation);
                    }
                } else {
                    let mut subtable =
                        Self::split_entry(translation, &chunk, entry, level);
    ...
    pub fn compact_subtables(&mut self, translation: &mut T) -> bool {
        ...
        for entry in &mut table.entries {
            if let Some(mut subtable) = entry.subtable(translation, self.level)
            {
                if subtable.compact_subtables(translation) {
