...
fn generate_for_std(args: GenCommandArgs, paths: &paths::ChromiumPaths) -> Result<()> {
    ...
    let template_path = paths
        ...
        .parent()
        .unwrap()
        .join(&config.gn_config.build_file_template);
    ...
    std::fs::write(
        paths
            .strip_template(paths.std_fake_root_config_template)
            .unwrap(),
        cargo_config,
    )
    .unwrap();
    ...
    std::fs::write(
        paths
            .strip_template(paths.std_fake_root_cargo_template)
            .unwrap(),
        cargo_toml,
    )
    .unwrap();
    ...
    dependencies.retain(|dep| {
        dep.dependency_kinds
            .contains_key(&deps::DependencyKind::Normal)
    });
    ...
    for dep in dependencies.iter_mut() {
        ...
        if let Some(lib) = dep.lib_target.as_mut() {
            ...
            if let Ok(remain) = lib
                .root
                .canonicalize()
                .unwrap()
                .strip_prefix(&rust_src_root)
            {
    ...
    let third_party_deps = dependencies
        .iter()
...
fn generate_for_third_party(args: GenCommandArgs, paths: &paths::ChromiumPaths) -> Result<()> {
    ...
    let handlebars = init_handlebars(&template_path)?;
    println!(
        "Generating third-party GN rules from {}",
    ...
    for dep in dependencies.iter_mut() {
        ...
        let per: Option<&Vec<String>> = config
            .per_crate_config
            .get(&dep.package_name)
            .map(|config| &config.remove_deps);
...
fn write_build_file(path: &Path, content: String) -> Result<()> {
    ...
    write!(
        io::BufWriter::new(child.stdin.take().unwrap()),
        "{}",
        content
    )
