...
pub struct DepOfDep {
    ...
    #[cfg(feature = "blueos")]
    pub lib_target: Vec<Option<LibTarget>>,
}
...
pub fn collect_dependencies(
    ...
    for (id, dep) in dependencies.iter_mut() {
            ...
            match target_type {
                ...
                TargetType::Bin => {
                    if allowed_bin_targets.contains(target.name.as_str()) {
                        dep.bin_targets.push(BinTarget {
                            root: src_root,
        ...
        for node_dep in iter_node_deps(node) {
            ...
            #[cfg(feature = "blueos")]
            let mut lib_target: Vec<Option<LibTarget>> = vec![];
            #[cfg(feature = "blueos")]
            for target in dep_pkg.targets.iter() {
                let target_type = match target.kind.iter().find_map(|s| TargetType::from_name(s)) {
                    Some(target_type) => target_type,
                    // Skip other targets, such as test, example, etc.
                    None => continue,
                ...
                match target_type {
                    TargetType::Lib(lib_type) => {
                        lib_target.push(Some(LibTarget {
                            root: src_root,
                            lib_type,
                        }));
                    }
...
fn explore_node<'a>(state: &mut TraversalState<'a>, node: &'a cargo_metadata::Node) {
    ...
    for dep_edge in iter_node_deps(node) {
        ...
        let dep: &mut Package = state
            .dependencies
            .entry(dep_edge.pkg)
            .or_insert_with(|| init_dep(state.path.clone()));
        let info: &mut PerKindInfo =
            dep.dependency_kinds
                .entry(dep_edge.kind)
                .or_insert(PerKindInfo {
...
mod tests {
    ...
    fn collect_dependencies_on_sample_output() {
        ...
        assert_eq!(
            dependencies[i]
                .dependency_kinds
                .get(&DependencyKind::Development)
                .unwrap()
                .features,
            empty_str_slice
        );
    ...
    fn exclude_dependency() {
        ...
        let deps_with_exclude = collect_dependencies(
            &metadata,
            None,
            Some(vec!["serde_derive".to_string()]),
            &config,
        );
        let deps_without_exclude = collect_dependencies(&metadata, None, None, &config);

        let pkgs_with_exclude: HashSet<&str> = deps_with_exclude
            .iter()
            .map(|dep| dep.package_name.as_str())
            .collect();
        let pkgs_without_exclude: HashSet<&str> = deps_without_exclude
            .iter()
            .map(|dep| dep.package_name.as_str())
            .collect();
        let mut diff: Vec<&str> = pkgs_without_exclude
            .difference(&pkgs_with_exclude)
            .copied()
            .collect();
        diff.sort_unstable();
        assert_eq!(
