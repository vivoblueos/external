...
mod tests {
    ...
    fn platform_is_supported() {
        for named_platform in SUPPORTED_NAMED_PLATFORMS {
            assert!(matches_supported_target(&Platform::Name(
                named_platform.to_string()
            )));
        }
        ...
        for os in supported_os_cfgs() {
            assert!(matches_supported_target(&Platform::Cfg(CfgExpr::Value(
                os.clone()
            ))));
        }
    ...
    fn windows_target_cfgs() {
        ...
        assert_eq!(
            filter_unsupported_platform_terms(Platform::Cfg(CfgExpr::from_str(cfg).unwrap())),
            Some(Platform::Cfg(
                CfgExpr::from_str("target_arch = \"x86_64\"").unwrap()
            ))
        );
        ...
        assert_eq!(
            filter_unsupported_platform_terms(Platform::Cfg(CfgExpr::from_str(cfg).unwrap())),
            Some(Platform::Cfg(
                CfgExpr::from_str("target_arch = \"x86\"").unwrap()
            ))
        );
