...
fn vendor_impl(args: VendorCommandArgs, paths: &paths::ChromiumPaths) -> Result<()> {
    ...
    for p in packages.values() {
        ...
        if config.resolve.remove_crates.contains(&p.name) {
            println!(
                "Generating placeholder for removed crate {}-{}",
                p.name, p.version
            );
    ...
    let vet_config_toml = create_vet_config(
        packages.values().copied(),
        &config,
        find_group,
        find_shipped,
    )?;
...

    {
        ...
        let file = std::fs::File::create(paths.vet_config_file).with_context(|| {
            format!(
                "Could not create vet config output file {}",
    ...
    for (dir, readme_file) in &all_readme_files {
        ...
        {
            file_path = dir.join("README.chromium");
        }
        #[cfg(feature = "blueos")]
        {
            file_path = dir.join("README.blueos");
        }
        let file = std::fs::File::create(&file_path).with_context(|| {
            #[cfg(not(feature = "blueos"))]
            {
                format!(
                    "Could not create README.chromium output file {}",
                    file_path.to_string_lossy()
                )
            }
            #[cfg(feature = "blueos")]
            {
                format!(
                    "Could not create README.blueos output file {}",
                    file_path.to_string_lossy()
                )
            }
        })?;
...
fn apply_patches(
    ...
    for (path, contents) in patches_contents {
        ...
        if let Err(e) = run_command(c, "patch", Some(&contents)) {
            log::error!(
                "Applying patches failed! Removing the {} directory.",
                crate_dir.display()
            );
...
fn placeholder_crate(
    ...
    let dependencies = node
        ...
        .map(|d| {
            let d_normalized = NormalizedName::from_crate_name(&d.name);
            let feature_dep_info = package
                .dependencies
                .iter()
                .find(|f| {
                    let f_normalized = NormalizedName::from_crate_name(&f.name);
                    f_normalized == d_normalized
                })
    ...
    let placeholder_crate = PlaceholderCrate {
        name: &package.name,
...

    {
        ...
        let file = std::fs::File::create(&file_path).with_context(|| {
            format!(
                "Could not create Cargo.toml output file {}",
                file_path.to_string_lossy()
            )
        })?;
        use std::io::Write;
    ...
    create_dirs_if_needed(&crate_dir.join("src")).context("creating src dir")?;
    {
        ...
        let file = std::fs::File::create(&file_path).with_context(|| {
            format!(
                "Could not create lib.rs output file {}",
                file_path.to_string_lossy()
            )
        })?;
        use std::io::Write;
