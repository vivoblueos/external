...
fn document_features_impl(args: &Args) -> Result<TokenStream, TokenStream> {
    let path = match std::env::var("CARGO_MANIFEST_DIR") {
        Ok(path) => path,
        Err(_) => {
            // CARGO_MANIFEST_DIR is not set (e.g. GN build), return empty docs
            return Ok(std::iter::once(proc_macro::TokenTree::from(
                proc_macro::Literal::string(""),
            ))
            .collect());
        }
    };
    let mut cargo_toml = match std::fs::read_to_string(Path::new(&path).join("Cargo.toml")) {
        Ok(content) => content,
        Err(_) => {
            // Cargo.toml not found at CARGO_MANIFEST_DIR (e.g. GN build with bad CWD),
            // return empty docs
            return Ok(std::iter::once(proc_macro::TokenTree::from(
                proc_macro::Literal::string(""),
            ))
