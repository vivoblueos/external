...
use core::{future::Future, future::poll_fn, marker::PhantomData, ptr::NonNull, task::Context};
...
impl<'t, T: Sync + Send> Handle<'t, T> {
    ...
    pub fn wait<'a>(&'a mut self) -> impl Future<Output = Status> + use<'a, 't, T> {
        poll_fn(|ctx| {
    ...
    T: Sync + Send,
{
    ...
    pub fn stop(self) -> impl Future<Output = ()> + use<'t, D, T> {
